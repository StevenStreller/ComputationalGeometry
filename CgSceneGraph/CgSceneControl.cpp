...
void CgSceneControl::calculatePickRay(double x, double y)
{
  ...
  if(m_pointcloud != nullptr){
      int pointIndex = m_pointcloud->pickNearestPoint(raystart, rayend, kNearestValue, showSmoothingMesh, &m_triangle_mesh, smoothingBaseFunction);
      m_pointcloud->markKNearest(pointIndex, kNearestValue);
      m_renderer->init(m_pointcloud);
  } else if(m_he_mesh != nullptr){
      m_he_mesh->pickNearestPointAndVisualizeLeastSquares(raystart, rayend, showSmoothingMesh, smoothingBaseFunction, &m_triangle_mesh);
...
void CgSceneControl::handleEvent(CgBaseEvent* e)
{
    ...
    {
        CgButtonClickEvent* buttonEvent = (CgButtonClickEvent*)e;
        if(buttonEvent->button() == CgButtonClickEvent::SubdivideButton) {
            m_he_mesh->loopSubdivide();
            m_renderer->init(m_he_mesh);
            m_renderer->redraw();
        }
        else if(buttonEvent->button() == CgButtonClickEvent::SmoothButton){
            if(m_he_mesh != nullptr){
                m_he_mesh->smoothWithLeastSquares(smoothingBaseFunction);
                m_renderer->init(m_he_mesh);
            }
            if(m_pointcloud != nullptr){
                m_pointcloud->smoothWithLeastSquares(kNearestValue, smoothingBaseFunction);
                m_renderer->init(m_pointcloud);
            }
            m_renderer->redraw();
        }
    ...
    if(e->getType() & Cg::CgSpinBoxChangedEvent){
        if(m_pointcloud != nullptr){

            CgSpinBoxChangedEvent* spinBoxEvent = (CgSpinBoxChangedEvent*)e;
            if(spinBoxEvent->spinBox() == CgSpinBoxChangedEvent::kIndex){

                kNearestIndex = spinBoxEvent->getValue();
                m_pointcloud->markKNearest(kNearestIndex, kNearestValue);
            }
            if(spinBoxEvent->spinBox() == CgSpinBoxChangedEvent::kValue){

                kNearestValue = spinBoxEvent->getValue();
                m_pointcloud->markKNearest(kNearestIndex, kNearestValue);
            }
            if(spinBoxEvent->spinBox() == CgSpinBoxChangedEvent::kdDepth){
                kdDepthValue = spinBoxEvent->getValue();
                if(m_triangle_mesh != nullptr){
                    delete m_triangle_mesh;
                    m_triangle_mesh = nullptr;
                }
                m_triangle_mesh = m_pointcloud->drawKdTree(kdDepthValue);
                if(m_triangle_mesh != nullptr){
                    m_renderer->init(m_triangle_mesh);
                    m_renderer->redraw();
                }
    ...
    if(e->getType() & Cg::CgCheckBoxClickEvent){
        CgCheckboxClickedEvent* checkboxEvent = (CgCheckboxClickedEvent*)e;
        if(checkboxEvent->checkbox() == CgCheckboxClickedEvent::showSmoothingMesh){
            showSmoothingMesh = checkboxEvent->isChecked();
            if(!showSmoothingMesh && m_triangle_mesh != nullptr && (m_he_mesh != nullptr || m_pointcloud != nullptr)){
                delete m_triangle_mesh;
                m_triangle_mesh = nullptr;
            }
            m_renderer->redraw();
    ...
    if(e->getType() & Cg::CgRadioButtonActivateEvent){
        CgRadioButtonClickedEvent* radioButtonEvent = (CgRadioButtonClickedEvent*)e;
        if(radioButtonEvent->radioButton() == CgRadioButtonClickedEvent::least_smoothing_biquadratic)
            smoothingBaseFunction = CgLeastSquares::biquadratic;
        else if(radioButtonEvent->radioButton() == CgRadioButtonClickedEvent::least_smoothing_bicubic)
            smoothingBaseFunction = CgLeastSquares::bicubic;
    }
